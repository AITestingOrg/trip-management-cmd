...
public abstract class ApiService<T, K> {
    ...
    public T getOne(String path) {
        ...
        ResponseEntity<T> exchange = this.restTemplate.exchange(uri, HttpMethod.GET, null,
    ...
    public List<T> getMany(String path) {
        String uri = buildUri(service.name(), path);
        logger.info("Sending a GET request to " + uri);
        ResponseEntity<List<T>> exchange = this.restTemplate.exchange(uri, HttpMethod.GET, null,
    ...
    public T create(String path, K obj) {
        ...
        ResponseEntity<String> exchange = this.restTemplate.exchange(uri, HttpMethod.POST, entity, String.class);
    ...
    private String buildUri(String appName, String path) {
        // this method does not use URI since the app_name is usually not a valid URI
        // Hostname
        String url;
        if (useSsl) {
            url = "https://";
        ...
        if (!appName.toLowerCase().contains("calculation")) {
